  ...
  return function (options) {
    ...
    return context.each(function () {
      ...
      local.renderer = function () {
        ...
        var firstWeekday = firstDate.day() - context.settings.week;
        var lastWeekday = lastDate.day() - context.settings.week;
        ...
        var _loop = function _loop(_i2) {
          ...
          var iDateFormat = iDate.format('YYYY-MM-DD');
              var $unit = $(helper.format('<div class="{0} {0}-date {0}-{3}" data-date="{1}"><a href="#">{2}</a></div>', helper.getSubClass('unit'), iDate.format('YYYY-MM-DD'), _i2, global.languages.weeks.en[iDate.day()].toLowerCase()));
          if (context.settings.enabledDates.length > 0) {
            if ($.inArray(iDateFormat, context.settings.enabledDates) === -1) {
              $unit.addClass(helper.getSubClass('unitDisabled'));
            }
          } else if (context.settings.disabledWeekdays.length > 0 && $.inArray(iDate.day(), context.settings.disabledWeekdays) !== -1) {
