...
class ResUsers(models.Model):
    ...
    def create(self, values):
        if not values.get('image_1920', False) and values.get('login', False) not in ['portaltemplate', 'default']:
            partner_obj = self.env['res.partner']
            name_order = partner_obj._get_names_order()
            if values.get('partner_id', False):
                partner_id = partner_obj.browse(values.get('partner_id'))
                if name_order == 'first_last':
                    name = '%s %s' % (partner_id.firstname, partner_id.lastname)
                else:
                    name = '%s %s' % (partner_id.lastname, partner_id.firstname)
            elif values.get('lastname', False) and values.get('firstname', False):
                if name_order == 'first_last':
    ...
    def write(self, values):
        for rec in self:
            if not values.get('image_1920', rec.image_1920) and values.get('login', rec.login) not in ['portaltemplate',
                                                                                                       'default']:
    ...
    @api.model
    def _delete_default_image_for_template(self):
        default_users = self.env['res.users'].search(
            [('login', 'in', ['portaltemplate', 'default']), ('active', '=', False)])
