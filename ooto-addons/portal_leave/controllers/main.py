...
class CustomerPortal(CustomerPortal):
    ...
    def create_new_leave_request(self, page=1, sortby=None, search=None, search_in='content', **kw):
        ...
        # if kw.get('state_id') == "Leave Type...":
        #     return request.render("portal_leave.registration_leave", {
        #         'leave_types': request.env['hr.leave.type'].sudo().search([]),
        #         'leave_type_info': True,
        #     })
        ...
        for employee in employees:
            ...
            if kw.get('halfday'):
                ...
                if leave.date_from and leave.date_to:
                    leave.write({'number_of_days' : leave._get_number_of_days(leave.date_from, leave.date_to, leave.employee_id.id)})
                else:
                    leave.write({'number_of_days' : 0.5})
    ...
    def edit_leave_request(self, page=1, sortby=None, search=None, search_in='content', **kw):
        ...
        if kw.get('halfday'):
            ...
            if leave_id.date_from and leave_id.date_to:
                leave_id.write(
                    {'number_of_days': leave_id._get_number_of_days(leave_id.date_from, leave_id.date_to, leave_id.employee_id.id)})
            else:
                leave_id.write({'number_of_days': 0.5})
